...
// Estos son los valores POR DEFECTO; el tema real que se pinta
// se carga desde Supabase (tabla wheel_theme) y puede ajustarse
// desde Configuración → Apariencia. Si no hay fila en la tabla
// todavía (o falla la carga), se usa esta paleta tal cual.
// ------------------------------------------------------------
...
// Etiquetas legibles para el panel de Apariencia
const THEME_FIELD_LABELS = {
  bg: "Fondo",
  bgCard: "Fondo de tarjetas",
  cream: "Crema (texto claro, ticket)",
  terracotta: "Terracota (botón principal)",
  gold: "Dorado (acentos, puntero)",
  green: "Verde (éxito)",
  ink: "Tinta (contornos, fondo oscuro)",
};
...
function computeSlices(prizes) {
  ...
  const count = visiblePrizes.length;
  // Gajos SIEMPRE del mismo tamaño visual (360° repartido en partes iguales),
  // independiente del peso/probabilidad real de cada premio. La probabilidad
  // real la sigue decidiendo el backend (fn_spin_wheel) según weight — esto
  // es puramente el dibujo. Es una decisión de diseño: no delatar visualmente
  // qué premio es más raro o más común.
  const angleSpan = count > 0 ? 360 / count : 360;
...
// ============================================================
function WheelCanvas({ prizes, spinning, targetAngle, onSpinEnd, colors }) {
...
function SpinView({ colors }) {
  ...
  async function handleSpin() {
    ...
    pendingResultRef.current = {
      ...
      celebration_message: winner.out_celebration_message,
    };
        ...
        <>
          ...
          {result && (
            <div className="ticket" role="status">
              <p className="ticket-message">{result.celebration_message}</p>
              <p className="ticket-claim-hint">Reclámalo con este código</p>
...
// ============================================================
function ConfigView({ session, colors, onThemeSaved }) {
  const [subTab, setSubTab] = useState("premios"); // premios | apariencia
  const [prizes, setPrizes] = useState([]);
  const [loading, setLoading] = useState(true);
      ...
      for (const p of toUpdate) {
        const { error } = await supabase
          ...
          .update({
            ...
            celebration_message: p.celebration_message || null,
          })
          .eq("id", p.id);
  ...
  return (
    <div className="config-view">
      <div className="subtabs">
        <button
          className={classNames("subtab-btn", subTab === "premios" && "subtab-btn-active")}
          onClick={() => setSubTab("premios")}
        >
          Premios
        </button>
        <button
          className={classNames("subtab-btn", subTab === "apariencia" && "subtab-btn-active")}
          onClick={() => setSubTab("apariencia")}
        >
          Apariencia
        </button>
      </div>

      {subTab === "apariencia" && <ThemeEditor colors={colors} onThemeSaved={onThemeSaved} />}
      ...
      <div className="prize-table">
        ...
        {prizes.map((p) => (
          <div key={p.id} className={classNames("prize-card", !p.active && "prize-row-inactive")}>
            ...
            <div className="prize-message-row">
              <label className="prize-message-label">Mensaje al ganar</label>
              <input
                type="text"
                value={p.celebration_message || ""}
                onChange={(e) => updateLocalPrize(p.id, { celebration_message: e.target.value })}
                placeholder={`GANASTE: ${p.name.toUpperCase()}`}
                className="input-text"
                maxLength={80}
              />
            </div>
...
// ============================================================
// ============================================================
// SUBVISTA: Editor de apariencia (paleta general de colores)
// ============================================================
function ThemeEditor({ colors, onThemeSaved }) {
  const [draft, setDraft] = useState(colors);
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState(null);

  // Si el tema cargado desde el padre cambia (p.ej. al entrar a esta
  // vista por primera vez, después de que App termine de cargarlo),
  // sincronizamos el borrador — pero solo si el usuario no tiene ya
  // cambios sin guardar, para no pisarle ediciones en curso.
  const [dirty, setDirty] = useState(false);
  useEffect(() => {
    if (!dirty) setDraft(colors);
  ...
  async function handleSave() {
    ...
    const { error } = await supabase
      ...
      .update({
        bg: draft.bg,
        bg_card: draft.bgCard,
        cream: draft.cream,
        terracotta: draft.terracotta,
        gold: draft.gold,
        green: draft.green,
        ink: draft.ink,
      })
    ...
    if (error) {
      setMessage({
        type: "error",
        text: `No se pudo guardar: ${error.message}. Si el error menciona que la tabla no existe, falta correr la migración 002_tema_configurable.sql en Supabase.`,
      });
    } else {
      setMessage({ type: "ok", text: "Colores guardados. Así se verá la app para todos." });
      setDirty(false);
      onThemeSaved?.();
    }
    setSaving(false);
  ...
  const fields = ["bg", "bgCard", "cream", "terracotta", "gold", "green", "ink"];
  ...
  return (
    <div className="theme-editor">
      <div className="config-head">
        <h2>Apariencia</h2>
        <p className="muted">
          Colores generales de toda la app. El color de cada premio en la ruleta se edita en la
          pestaña "Premios".
        </p>
      </div>
      ...
      <div className="theme-fields">
        {fields.map((field) => (
          <div key={field} className="theme-field-row">
            <input
              type="color"
              value={draft[field]}
              ...
              className="input-color"
            />
            <span className="theme-field-label">{THEME_FIELD_LABELS[field]}</span>
            <span className="theme-field-hex">{draft[field]}</span>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  const loadTheme = useCallback(async () => {
    const { data, error } = await supabase.from("wheel_theme").select("*").eq("id", 1).maybeSingle();
    if (!error && data) {
      setColors({
        bg: data.bg,
        bgCard: data.bg_card,
        cream: data.cream,
        terracotta: data.terracotta,
        gold: data.gold,
        green: data.green,
        ink: data.ink,
      });
    }
    // Si hay error (p.ej. la tabla todavía no existe porque no se corrió
    // la migración) o no hay fila, se queda con DEFAULT_COLORS — la app
    // sigue funcionando igual, solo sin personalización de paleta.
    setThemeLoaded(true);
  }, []);
  ...
  return (
    <div className="app-root">
      ...
      <main className="app-main">
        {tab === "ruleta" && <SpinView colors={colors} />}
        {tab === "config" &&
          (session ? (
            <ConfigView session={session} colors={colors} onThemeSaved={loadTheme} />
          ) : (
...
.ticket-claim-hint { font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: rgba(23,19,16,0.55); margin: 0; }
.ticket-code {
  font-family: 'Archivo Black', sans-serif;
  font-size: 26px;
...
.subtab-btn-active { background: ${colors.gold}; border-color: ${colors.gold}; color: ${colors.ink}; }

.theme-fields { display: flex; flex-direction: column; gap: 4px; margin-bottom: 22px; }
.theme-field-row {
  display: flex;
  align-items: center;
  gap: 12px;
...
.theme-field-label { flex: 1; font-size: 14px; }
.theme-field-hex { font-size: 12px; color: rgba(242,232,213,0.45); font-family: monospace; }

.prize-table { display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; }
.prize-card {
  background: ${colors.bgCard};
  border-radius: 8px;
  padding: 10px;
...
@media (max-width: 420px) {
  ...
  .prize-message-row { flex-direction: column; align-items: flex-start; }
}
`;
}
